...
class HandleApprove extends React.Component {
  ...
  async updateZoomLink(id, zoomLink, hostLink, pass) {
    ...
    var approvedEvents = await db.collection("events")
      ...
      .then(() => {
        this.setState({
          response: "Zoom link created and zoom link put into database successfully! "
            + "\nJoin url: " + zoomLink
            + "\n Host Link: " + hostLink
            + "\n PW: " + pass
        ...
        this.setState({
          response: "Zoom link created but could not put into event in database."
            + "\nJoin url: " + zoomLink
            + "\n Host Link: " + hostLink
            + "\n PW: " + pass
            ...
            if (event === 0) {
              ...
              if (event.zoomLink === true) {
                ...
                Axios.post(requestUrl, options)
                  .then(res => {
                    this.setState({
                      response: "Success. Created zoom meeting:"
            ...
            </td>
            {
              this.state.headings.map((num, j) => {
                if (num === "tags") {
                  return (<td key={j}
                    style={{
                      border: "1px solid #dddddd", textAlign: "left", width: "50px",
